...
⚠️ **KEY**: When tasks reuse existing code, always EXTEND (add new) rather than MODIFY (change old):

- ✅ **Safe**: "Add new method `[NewMethod]()` to [ExistingService]"
- ✅ **Safe**: "Add `[newColumn]` column to [ExistingTable]"
- ✅ **Safe**: "Create new class [NewClass] using existing [ExistingService]"
- ❌ **Risky**: "Modify `[ExistingMethod]()` signature to add [parameter]"
- ❌ **Risky**: "Edit `[ExistingMethod]()` to change behavior when [condition]"

**Task Description Rule**: If task reuses existing code, include note:
> ⚠️ **Regression Protection**: Do not modify existing [Component] signatures/behavior. Add new methods or fields only. Existing tests and callers depend on unchanged contracts.
...
- [ ] T004 Scan codebase for reusable components and patterns

  **Scope**:
  - **Backend**: Services, models/entities, repositories, DTOs, validation, error handling, logging patterns, configuration, constants, test fixtures
  - **React UI** (if applicable): Components, custom hooks, UI library/design system, state management (Redux/Context/Zustand), API client utilities, form patterns, TypeScript types, styling utilities
  
  **Deliverable**: Add Reuse Analysis section to plan.md mapping each feature requirement → existing component → reuse decision (yes/no/extend) + justification. Flag conflicts or refactoring needs.
